...
def get_token_desc(lang="ru"):
    if lang == "ru":
        return """
        Теперь перейди в @botfather, создай бота и пришли мне токен.
        Токен выглядит вот так:
        Подробная инструкция по созданию бота тут <ссылка>
        """
...
tr = {
    ...
    "Теперь создадим первую задачу.\nПросто зайдите в пункт меню “Добавить задачу” и там все поймете.": "Yet let's create our first task. Navigate to “Add task” in the menu and you'll get it",
    "Мои боты🤖": "My bots🤖",
    ...
    "У вас пока нет действующей таблицы. Привяжите её, чтобы получать статистику и ответы сотрудников.": "You haven't linked a spreadsheet yet (spreadsheets.google.com). Link one to start receiving employee answers.",
